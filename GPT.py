...
class MultiHeadAttention(nn.Module):
	def __init__(self, d_in, d_out, context_length, dropuot, num_heads, qkv_bias=False):
		super().__init__() 
		...
		self.out_proj = nn.Line(d_out, d_out)
	...
	def forward(self, x):
		b, num_tokens, _ = s.shape 	# Shape: (b, num_tokens, d_out)
...
class GPTModel(nn.Module):
	...
	def forward(self, in_idx):
		...
		# if the seq_len is smaller than the context_length, seq_len is use.
...
def calc_loss_loader(data_loader, model, device, num_batches=None):
	...
	for i, (input_batches, target_batches) in enumerate(data_loader):
		if i < num_batches:
...
def train_model_simple(model, train_loader, val_loader, optimizer, device, num_epocs, eval_freq, eval_iter, start_context, tokenizer):
	...
	for epoch in range(num_epocs):
		for input_batch, target_batch in train_loader:
			...
			loss_calc_loos_batch(input_batch, targer_batch, model, device)
